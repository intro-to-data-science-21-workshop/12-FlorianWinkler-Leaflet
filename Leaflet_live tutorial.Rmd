...
Are you ready to create this interactive map of COVID cases across Europe in just under 20 minutes? 😎

![](motivation.png "covid cases europe")

Yes, you're right. This is not interactive. Well, it's just a screenshot (for now). Let's get started!
...
To show you a real-world example of how we can use Leaflet to visualize data in maps, we are going to work with COVID data from European countries. We've compiled a small data set from the [COVID-19 Data Repository](https://github.com/CSSEGISandData/COVID-19) hosted by the Johns Hopkins University (JHU) that can be downloaded from our GitHub workshop repository (credits to Riddhiman on the [R'tichoke blog](https://rtichoke.netlify.app/post/covid_visualisation_using_leaflet/) for providing a summarized version of the JHU data set).
...
We can use markers to call out points on a map. Marker locations are expressed in latitude/longitude coordinates, and can either appear as icons or as circles.

Icon markers are added using the `addMarkers` function. Their default appearance is a dropped pin.
...
As with most layer functions, the `label` option can be used to display a text label. A label is a textual or HTML content that can be attached to markers and shapes to be always displayed or displayed on mouse over. Unlike popups (see one step below), you don't need to click a marker for the label to be shown.
...
The `popup` argument can be used to add a message to be displayed on click. Popups are small boxes containing arbitrary HTML, that point to a specific point on the map.
...
cases_circles <- europe %>% 
  ...
  # Circle radius (arbitrary scaling function for dramatic effect)
  ...
  # Circle markers that change size according to confirmed cases
...
The great thing about leaflet is that maps are interactive: Try clicking on one of the circle markers. What was the fatality ratio in France in September 2021? Did Denmark or Hungary have more COVID cases in September?
